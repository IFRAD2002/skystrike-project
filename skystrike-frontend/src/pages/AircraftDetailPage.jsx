                  ...
                  <table className="table table-pin-rows">
                    ...
                    <tbody>
                      {maintenanceLogs.length > 0 ? maintenanceLogs.map(log => (
                        <tr key={log._id}><td>{new Date(log.serviceDate).toLocaleDateString()}</td><td>{log.description}</td></tr>
                      )) : <tr><td colSpan="2" className='text-center'>No maintenance logs found.</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
            ...
            {activeTab === 'schedule' && (
              <div className="card-body">
                <div className="flex justify-between items-center">
                    ...
                    {userRole && <button className="btn btn-secondary btn-sm" onClick={() => document.getElementById('schedule_maintenance_modal').showModal()}>Schedule New</button>}
                </div>
                <div className="divider my-2"></div>
                {aircraft.scheduledMaintenanceDate ? (
                  <div className="alert shadow-lg">
                    <div>
                      <h3 className="font-bold text-lg">{new Date(aircraft.scheduledMaintenanceDate).toDateString()}</h3>
                      <div className="text-sm opacity-70 mt-1">{aircraft.scheduledMaintenanceNotes}</div>
                    </div>
                    {userRole && 
                      <div className="flex-none">
                        <button className="btn btn-primary btn-sm" onClick={() => document.getElementById('log_maintenance_modal').showModal()}>Log Work</button>
                      </div>
                    }
                  </div>
      ...
      <dialog id="log_maintenance_modal" className="modal">
        <div className="modal-box">
          ...
          <div className="modal-action">
            <button onClick={handleLogSubmit} className="btn btn-primary">Save Log</button>
